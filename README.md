...
| **watch-git** | `watches/watch-git.yml` | Force push, branch -D, and other destructive git ops (block); commit, push, reset --hard, force-with-lease, and other mutating ops (ask). Stage-only ops (add, rm, non-hard reset) are allowed |
