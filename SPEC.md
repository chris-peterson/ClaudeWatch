...
- **[RL-16]** The engine shall ship the `unless_condition` predicate `is_relative_to_cwd`: it matches (skipping the ask) when the input is a `bash` `rm` command whose every deletion target resolves strictly under the working directory (`cwd`) supplied in the hook input. The check shall be pure string resolution of the command's target paths against `cwd` — no filesystem access — so the decision stays deterministic; `cwd` is a deterministic input on stdin, so the core contract ([EN] determinism) holds. The predicate shall decline (the ask stands) whenever in-tree-ness cannot be established from the command text: `cwd` is absent; the command is compound (handled by [OUT-08]); the program is not `rm`; there are no targets; a target is the working directory itself or a `.git` directory; a target carries an unresolvable marker (`~`, `$`, backtick, a glob `*`/`?`/`[`, or a `..` segment); or a target resolves outside `cwd`.
...
- **[SH-01]** The plugin shall ship the following rule sets:
  ...
  - **`watch-files`** — generic destructive filesystem operations. **Block** rules shall cover: `rm -rf /`, `rm -rf /*`, `chmod 777`, `mv … /dev/null`, and `shred`. **Ask** rules shall cover: recursive `rm -rf`, recursive `rm -r`, `mv` from a root-level path, `chmod`, and `chown`. The two recursive-rm ask rules shall carry an `unless_regex` exempting paths under `~/.cache/`, `/tmp/`, or `/var/tmp/`, and an `unless_condition` of `is_relative_to_cwd` ([RL-15], [RL-16]), so a recursive delete confined to the working tree — recoverable from git history — is allowed, while one reaching outside it still prompts.
