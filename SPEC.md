...
- **[OUT-04]** When no block rule matches and at least one ask rule matches in any rule set, the engine shall emit a single `ask` decision aggregating all ask reasons, separated by newlines — subject to the compound-command escalation in [OUT-08]. The decision shall be a function of the command alone: the same input grades the same way whatever `permission_mode` the host reports, and the engine shall not read that field on the decision path ([LOG-03] logs it; nothing consumes it). **Whether an emitted `ask` reaches a person is the host's half of the contract and is not guaranteed.** Two behaviors are established: an interactive session in `auto` clears the call before the hook's `ask` has a prompt surface, so the command runs unconfirmed ([claude-code#89561](https://github.com/anthropics/claude-code/issues/89561)); a headless `--print` session has nobody to answer, so the call fails closed and is denied. The engine cannot distinguish these — `permission_mode` reads `auto` in both the prompted and the prompt-suppressed case, and `--permission-prompts none` sets no field of its own. An `ask` is therefore a *request* for confirmation, never evidence one happened, and a rule guarding something unrecoverable belongs in the block tier rather than relying on the prompt.
...
- **[SK-14]** The skill shall present three groups: **allow candidates** (frequently-allowed commands not covered by the current allow list), **ask candidates** (commands ClaudeWatch repeatedly asks about, with the matched rule), and a **deny summary** (blocked commands grouped by reason, informational). An allow candidate shall name the tool its records came from and propose a permission rule for that tool (`Bash(…)` or `Monitor(…)`), and shall be judged already-covered only against existing rules for the same tool: the host treats the two as separate rule families, so a `Bash` rule does not cover the same command issued through `Monitor`. The coverage judgment approximates the host's matcher and shall err toward proposing, since a redundant proposal costs the user a line to read and a wrong suppression costs them a candidate they never see: a rule's literal shall cover a command shape only where it lands on a token boundary, and a rule whose wildcard precedes further pattern text (`Bash(git * main)`) shall cover nothing.
