...
- **[SH-01]** The plugin shall ship the following rule sets:
  ...
  - **`watch-git`** — destructive git operations. **Block** rules shall cover: `git push --force`, `git reset --hard`, `git checkout .`, `git checkout -- <file>`, `git restore .`, `git clean -f`, `git branch -D`, `git stash drop`, `git stash clear`, and `git reflog expire/delete`. **Ask** rules shall cover: `git add`, `git rm`, `git rm --cached`, `git reset`, `git commit`, `git stash`, and `git push`. Each rule shall match through git's pre-subcommand global flags (`-C <path>`, `-c <key>=<value>`, `--git-dir[=]<path>`, `-P`), including quoted values containing spaces, so that invocations like `git -C /repo push --force` are not silently bypassed.
