...
`ClaudeWatch` solves this by intercepting every `Bash` tool call — and the code Claude writes to disk via `Write`/`Edit` — and matching against regex rules loaded from YAML config files. The engine auto-discovers all `*.yml` files in the `rules/` directory. Each file is a *rule set* — a group of patterns guarding one domain (git, secrets, package installs, and so on) — so adding protection for a new domain means dropping in another YAML file, with no engine change.
