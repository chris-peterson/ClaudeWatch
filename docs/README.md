...
`ClaudeWatch` solves this by intercepting every `Bash` and `Monitor` tool call (and the code Claude writes to disk via `Write`/`Edit`) and matching against regex rules loaded from YAML config files. The engine auto-discovers all `*.yml` files in the `watches/` directory. Each file is a *rule set*, a group of patterns guarding one domain (git, secrets, package installs, and so on), so adding protection for a new domain means dropping in another YAML file, with no engine change.
...
Most of the time ClaudeWatch stays out of the way. To look under the hood (see what's guarding your commands, or tune the rules from what's been prompting you), reach for the skills:
