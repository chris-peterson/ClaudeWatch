...
chain, a `$(…)`, a `( … )` subshell, or a `<( … )` process substitution. That
keeps the command out of the hard block; whether it then reaches a person is
the separate question the ask-tier note covers. Its output is usually short
enough that the `| tail` bought you nothing. When you need a value from one
command in the next, run the first, read its result, then use it in a second
call. Pipes between plainly-safe commands (`grep … | head`) stay fine — the
escalation fires only when a guarded command is in the chain.
