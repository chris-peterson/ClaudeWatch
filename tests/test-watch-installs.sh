...
# by whitespace. Anchoring on `\s` or `$` alone let these through as allow, so
# the compound escalation never saw an ask to raise.
...
# commit-tree` out; the anchor it replaced missed it too.
